function varargout = DippingInterface(varargin)
% DIPPINGINTERFACE MATLAB code for DippingInterface.fig
%      DIPPINGINTERFACE, by itself, creates a new DIPPINGINTERFACE or raises the existing
%      singleton*.
%
%      H = DIPPINGINTERFACE returns the handle to a new DIPPINGINTERFACE or the handle to
%      the existing singleton*.
%
%      DIPPINGINTERFACE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DIPPINGINTERFACE.M with the given input arguments.
%
%      DIPPINGINTERFACE('Property','Value',...) creates a new DIPPINGINTERFACE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before DippingInterface_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to DippingInterface_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help DippingInterface

% Last Modified by GUIDE v2.5 27-Jun-2016 19:39:29

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @DippingInterface_OpeningFcn, ...
                   'gui_OutputFcn',  @DippingInterface_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before DippingInterface is made visible.
function DippingInterface_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to DippingInterface (see VARARGIN)

% Choose default command line output for DippingInterface
...
% so window can get raised using DippingInterface.

% SET UP SAMPLE DATA - MODIFY THIS INSTEAD TO LOAD a PICKS OBJ
handles.Pforward = PicksObj('picks206.mat',54.5);
handles.Preverse = PicksObj('picks212.mat',78.5);
...
% INITIATE SLIDER RANGES AND VALUES ***************************************
    ...
    set(handles.V1AveLabel,'Visible','off');
...
% average v1
handles.V1Ave = (handles.VForward1+handles.VReverse1)/2;
% dip angle - convert slopes to s/m; convert reverse slope to positive
% Burger et al. p. 99
handles.beta = (asin(handles.V1Ave*handles.slopeForwardRefract/1000.) - ...
                asin(handles.V1Ave*handles.slopeReverseRefract/-1000.))/2;
...
% critical angle
thetaic = (asin(handles.V1Ave*handles.slopeForwardRefract/1000.) + ...
                asin(handles.V1Ave*handles.slopeReverseRefract/-1000.))/2;
handles.V2 = handles.V1Ave/sin(thetaic);
    
handles.dForward = handles.TiForward/1000.*handles.V1Ave/...
                   (2*cos(thetaic)*cos(handles.beta));
               
handles.dReverse = handles.TiReverse/1000.*handles.V1Ave/...
                   (2*cos(thetaic)*cos(handles.beta));
...
handles.hV1Patch = patch(XV1Patch, YV1Patch,handles.V1Ave,'EdgeColor','none');
hold on;
% plot lower layer V2
...
set(handles.V1AveLabel,'Visible','on');
    caption = sprintf('%.0f', handles.V1Ave);
    set(handles.V1AveLabel,'String',['v1ave = ' caption ' m/s']);  
...
% hObject    handle to SaveToFileButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
function varargout = DippingInterface_OutputFcn(hObject, eventdata, handles)
...
% --- Executes during object creation, after setting all properties.
function V1AveLabel_CreateFcn(hObject, eventdata, handles)
% hObject    handle to V1AveLabel (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
