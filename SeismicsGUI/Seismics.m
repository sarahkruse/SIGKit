...
handles.TimePicks=load('picks303.mat')';
handles.xmin = min(handles.TimePicks.x);
handles.xmax = max(handles.TimePicks.x);
 plot(handles.pick_plot,handles.TimePicks.x,handles.TimePicks.t,'.k');
...
handles.isLayerThicknessLeft = 0;  handles.isLayerThicknessRight = 0;
...
if length(indrefleft) > 0
    ...
    plot(handles.pick_plot,handles.xRefractLeft,handles.tRefractLeft,'*m');
    ...
    if length(indrefleft) > 1 % need to points to fit to a line
        ...
        if (handles.doLinePlot == 1)
            ...
            handles.hLineRefractLeft = plot(handles.pick_plot,xl,tl,'m');    
...
if length(indrefright) > 0
    ...
    plot(handles.pick_plot,handles.xRefractRight,handles.tRefractRight,'*r');
    ...
    if length(indrefright) > 1 % need two points to fit to a line
        ...
        if (handles.doLinePlot == 1)
            ...
            handles.hLineRefractRight = plot(handles.pick_plot,xl,tl,'r'); 
...
if button_state == get(hObject,'Max') %button is pushed
    ...
    set(handles.toggle_struct_ray,'Visible','on');
    ...
    if  handles.isSlopeRefractRight == 1;
        ...
        tl(1) = handles.slopeRefractRight*xl(1)+handles.interRefractRight;
        tl(2) = handles.slopeRefractRight*xl(2)+handles.interRefractRight;
        handles.hLineRefractRight = plot(handles.pick_plot,xl,tl,'m'); 
         % add velocity label below plot
        set(handles.velocity_refract_right,'Visible','on'); 
        caption_vrefright = sprintf('%.0f', handles.vRefractRight);
        set(handles.velocity_refract_right,'String',...
...
elseif button_state == get(hObject,'Min')
    ...
    set(handles.toggle_struct_ray,'Visible','off');
...
% --- TOGGLE BUTTON TO SHOW STRUCTURE AND RAYS ----------------------------
 % --- Executes on button press in toggle_struct_ray.
function toggle_struct_ray_Callback(hObject, eventdata, handles)
button_state = get(hObject,'Value');
if button_state == get(hObject,'Max') %button is pushed
	handles.doRayPlot = 1;
elseif button_state == get(hObject,'Min')
...
% --- Executes during object creation, after setting all properties.
function toggle_struct_ray_CreateFcn(hObject, eventdata, handles)
% NOTE invisible in initial call
