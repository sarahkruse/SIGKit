classdef SeisObj
    ...
    %
    % Data fields are W (measured amplitude for each trace), RecXProf- relative
...
%     end
    ...
    methods
        function newSeisObj = SeisObj(varargin)
            ...
            %add the Data folder to the Matlab search path so that only the filename needs to be specified.
            path(path,Datapath);
...
%                 newSeisObj.Acquistion_date = Date;
            ...
            function plot(theSeisObj,varargin)
                ...
                %alternate value. Example plot(ST2,'filtered',50);
...
%                         unique(theSeisObj.SamplingInt)*1e3);
                    ...
                    title({['Record #', num2str(theSeisObj.FileNumber),'; ' ...
