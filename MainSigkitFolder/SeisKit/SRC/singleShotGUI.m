...
% Last Modified by GUIDE v2.5 12-Jul-2016 13:40:42
...
set(handles.V1LeftLabel,'Visible','off');
set(handles.V2LeftLabel,'Visible','off');
set(handles.V1RightLabel,'Visible','off');
set(handles.V2RightLabel,'Visible','off');
set(handles.TH1LLabel,'Visible','off');
set(handles.TH2LLabel,'Visible','off');
set(handles.TH1RLabel,'Visible','off');
set(handles.TH2RLabel,'Visible','off');
...
%     delete(handles.isLineDirectLeft);
    delete(handles.hLineDirectLeft);
    handles.isLineDirectLeft = 0;
end
if handles.isLineRefractLeft == 1
%     delete(handles.isLineRefractLeft);
...
%         XPatchRefractLeft = [x2RefLt CrossOverLeft CrossOverLeft x2RefLt];
         XPatchRefractLeft = [x2RefLt xshot xshot x2RefLt];
...
%                         (VelRefractRight-VelDirectRight));
        ...
        XPatchDirectRight = [xshot x2RefRt x2RefRt xshot ];
%         XPatchDirectRight = [x1DirRt x2RefRt x2RefRt x1DirRt];
        YPatchDirectRight = [0 0 -ThicknessRt1 -ThicknessRt1];
        patch(XPatchDirectRight, YPatchDirectRight,VelDirectRight,'EdgeColor','none'); hold on;
        XPatchRefractRight = [x2RefRt xshot xshot x2RefRt];
...
%     axes(handles.StructurePlot);
    ...
    mxD = max(D(:));
    minRecLoc = min(handles.obj.RecXProf);
    maxRecLoc = max(handles.obj.RecXProf);
    minY = -1*(maxRecLoc-minRecLoc)/2;
    ylim([ minY 1]);
%     ylim([ -round(mxD) 1]);
    caxis([0 2500]);
...
%     daspect([1 1/VertExag 1]);
    ...
    set(handles.TH1LLabel,'Visible','on');
%     set(handles.TH2LLabel,'Visible','on');
     set(handles.TH1RLabel,'Visible','on');
%       set(handles.TH2RLabel,'Visible','on');
    % This block may be incorrect
%     if VelDirectLeft == 0
...
%Grab the velocity and thickness data for both the left and the right sides
%of the model and add them to the structure panel
    if handles.isLineDirectLeft==1
        set(handles.V1LeftLabel,'Visible','on');
        caption = sprintf('%.0f', VelDirectLeft);
        
        set(handles.V1LeftLabel,'String',['V1 Left = ' caption ' m/s']);
    end
    
    if handles.isLineRefractLeft==1
        set(handles.V2LeftLabel,'Visible','on');
        caption = sprintf('%.0f',VelRefractLeft );
        
        set(handles.V2LeftLabel,'String',['V2 Left = ' caption ' m/s']);
    end
    
    if handles.isLineDirectLeft==1 || handles.isLineRefractLeft==1
        if VelRefractLeft == 0 && VelDirectLeft ~= 0
            caption ='?';
        set(handles.TH1LLabel,'String',['TH 1 Left = ' caption ' m']);   
        elseif VelRefractLeft ~= 0 && VelDirectLeft ~= 0
            caption = sprintf('%.0f', ThicknessLt1);
            set(handles.TH1LLabel,'String',['TH 1 Left = ' caption ' m']);
        end
    else
        set(handles.TH1LLabel,'Visible','off');
...
%         caption = '-';
%     else
%         caption = sprintf('%.0f', ThicknessLt2);
%     end
%     set(handles.TH2LLabel,'String',['TH 2 Left = ' caption ' m']);
    
    if handles.isLineDirectRight==1
        set(handles.V1RightLabel,'Visible','on');
        caption = sprintf('%.0f', VelDirectRight);
        set(handles.V1RightLabel,'String',['V1 Right = ' caption ' m/s']);  
    end
    
    if handles.isLineRefractRight==1
        set(handles.V2RightLabel,'Visible','on');
        caption = sprintf('%.0f',VelRefractRight );
        set(handles.V2RightLabel,'String',['V2 Right = ' caption ' m/s']);
    end
   
    
    if handles.isLineDirectRight==1 || handles.isLineRefractRight==1
        if VelRefractRight == 0 && VelDirectRight ~= 0
            caption = '?';
            set(handles.TH1RLabel,'String',['TH 1 Right = ' caption ' m']);
        elseif VelRefractRight ~= 0 && VelDirectRight ~= 0
            caption = sprintf('%.0f', ThicknessRt1);
            set(handles.TH1RLabel,'String',['TH 1 Right = ' caption ' m']);
        end
    else
        set(handles.TH1RLabel,'Visible','off');
...
%         caption='-';
%     else
%         caption = sprintf('%.0f', ThicknessRt2);
%     end
%     set(handles.TH2RLabel,'String',['TH 2 Right = ' caption ' m']);
...
if exist(Savepath,'dir')
    ...
    Filenamejpg = [Savepath,fs,num2str(handles.obj.FileNumber),'_Velocity_structure.jpg'];
    
    print(gcf,'-djpeg','-r300',Filenamejpg)
