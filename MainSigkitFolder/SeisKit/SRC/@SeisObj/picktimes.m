...
%             plot([F(index) F(index)],[1 -1], 'r');
            ...
            legend('Normalized Trace','Original Pick')
             ...
             waitfor(f,'Name');
%              if exist(btn1_gcbf,'var')
%                 [T(index),A,button(index)]=ginput(1);
%             elseif exist(btn2_gcbf,'var')
%                 T(index) = F2(index);
%                 button(index) = 1;
%             end
            [T(index),A,button(index)]=ginput(1);
...
%             f = figure('Position',[1 1 scrsz(3)*0.9 scrsz(4)*0.9],'Visible','off');
%             normTrace(:,Windex(i)) = Traces(:,Wiindex(i))/max(Traces(:,Windex(i)));
%             tmin = min(normTrace(:,Windex(i)));
%             tmax = max(normTrace(:,Windex(i)));
%             %swap min and max values for normalizing the traces
%             Tmin = tmax*-1;
%             Tmax = tmin*-1;
%             
%             subplot(2,1,1)
%             plot(t, (-1*Traces(:,WIndex(i)-1))/max(Traces(:,WIndex(i)-1)), 'k');
%             hold on;
...
%             
%             subplot(2,1,2)
%             plot(t, (-1*Traces(:,WIndex(i)))/max(Traces(:,WIndex(i))), 'k');
%             hold on;
